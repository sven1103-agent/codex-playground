...
## Reasoning + Tooling (Sequential MCP + Linear Thinking)

- First, call MCP tool **`sequentialthinking`** to generate the plan and checkpoints.
- Then, apply the **linear-thinking** skill to execute that plan as a strictly ordered sequence.
- Do not begin implementation until the tool-produced plan is complete and summarized as numbered steps.

## Skill + MCP Availability Check (MANDATORY)

Both of the following MUST be available:

- The `linear-thinking` skill
- The `sequentialthinking` MCP tool

Before beginning the task:
1. Confirm both are accessible.
2. If either is missing, STOP immediately and report which component is unavailable.
